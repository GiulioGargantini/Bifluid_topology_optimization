...
class Inflow_cold_boundary(SubDomain):
    def inside(self, x, on_boundary):
        return on_boundary and ((x[1] >= data.length_y_wall_hole_adim) and (x[1] <= data.length_y_wall_hole_adim + data.length_y_hole_adim)) and near(x[0], 0.0)

class Outflow_hot_boundary(SubDomain):
    def inside(self, x, on_boundary):
        return on_boundary and ((x[1] <= data.length_y_adim - data.length_y_wall_hole_adim) and (x[1] >= data.length_y_adim - data.length_y_wall_hole_adim - data.length_y_hole_adim)) and near(x[0], data.length_x_adim)

class Outflow_cold_boundary(SubDomain):
    def inside(self, x, on_boundary):
        return on_boundary and ((x[1] >= data.length_y_wall_hole_adim) and (x[1] <= data.length_y_wall_hole_adim + data.length_y_hole_adim)) and near(x[0], data.length_x_adim)

class Inflow_hot_boundary(SubDomain):
    def inside(self, x, on_boundary):
        return on_boundary and ((x[1] <= data.length_y_adim - data.length_y_wall_hole_adim) and (x[1] >= data.length_y_adim - data.length_y_wall_hole_adim - data.length_y_hole_adim)) and near(x[0], 0.0)
...
class class_IC_double_channel_eta1(UserExpression):
    def eval(self, values, x):
        values[0] = 0.0
        if ((x[1] >= data.length_y_wall_hole_adim - 0.02) and (x[1] <= data.length_y_wall_hole_adim + data.length_y_hole_adim + 0.02)):
            values[0] = 1.0
        elif(((data.length_y_adim - x[1]) >= data.length_y_wall_hole_adim - 0.02 ) and ((data.length_y_adim - x[1]) <= data.length_y_wall_hole_adim + data.length_y_hole_adim + 0.02)):
            values[0] = 1.0
...
if data.load_mesh == False:
    ...
    ic_eta2 = class_IC_partition_eta2(element=A.ufl_element())
