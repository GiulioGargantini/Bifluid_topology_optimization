    ...
    # BC for the velocity
        def eval(self, values, x):
            ...
            if ((x[0] <= DOLFIN_EPS) or (x[0] >= data.length_x_adim - DOLFIN_EPS)):
                if ((x[1] >= data.length_y_wall_hole_adim) and (x[1] <= data.length_y_wall_hole_adim + data.length_y_hole_adim)):
                    t = (x[1] - data.length_y_wall_hole_adim - data.length_y_hole_adim / 2) * 2 / data.length_y_hole_adim
                    values[0] =  data.u_max_in_adim * (1.0 - t**2)
                elif ((x[1] <= data.length_y_adim - data.length_y_wall_hole_adim) and (x[1] >= data.length_y_adim - data.length_y_wall_hole_adim - data.length_y_hole_adim)):
                    t = (data.length_y_adim - x[1] - data.length_y_wall_hole_adim - data.length_y_hole_adim / 2) * 2 / data.length_y_hole_adim
                    values[0] =   data.u_max_in_adim * (1.0 - t**2)
...
def define_cost_fct(eta1_a, eta2_a, u_a, p_a, T_a, eta2f_a, n_a):
    ...
    return cost + contiguity_penalization + velocity_penalization # + gradient_penalization
...
def save_and_compute_cost(eta1_opt, eta2_opt, iteration, save = True):
    ...
    #### Compute the cost
    J_tot = define_cost_fct(eta1_opt, eta2_opt, u_opt, p_opt, T_opt, eta2f_opt, n)
    J_val = combined_cost(eta1_opt, eta2_opt, u_opt, p_opt, T_opt, n)
    J_pre = pressure_cost(eta1_opt, eta2_opt, u_opt, p_opt, n)
    J_hea = thermal_cost(eta1_opt, eta2_opt, u_opt, p_opt, T_opt, n)
    J_penalization = penalization_cost(eta1_opt, eta2_opt, eta2f_opt)
    J_velocity = velocity_penalization_cost(u_opt, eta1_opt)
    avg_temperatures = avg_temp(T_opt)
    return J_tot, J_val, J_pre, J_hea, J_penalization, J_velocity, avg_temperatures
...
def avg_temp(T_opt):
    avg_T_cold_in = assemble(T_opt * ds(1)) / size_channels
    avg_T_hot_in = assemble(T_opt * ds(2)) / size_channels
    avg_T_cold_out = assemble(T_opt * ds(3)) / size_channels
    avg_T_hot_out = assemble(T_opt * ds(4)) / size_channels
...
size_channels = assemble(Constant(1.0) * ds(1, domain = meshes.mesh))
...
for j in range(0, len(data.b_alpha) + 1):
    ...
    J_tot, J_val, J_pre, J_hea, J_penalization, J_velocity, avg_temperatures = save_and_compute_cost(eta1_opt, eta2_opt, i, True)
    steps_vector.append(i)
    J_tots.append(J_tot)
    J_vals.append(J_val)
    J_pres.append(J_pre)# * data.const_adim_cost_pressure)
    J_thes.append(J_hea)# * data.const_adim_cost_thermal)
    J_penalizations.append(J_penalization)
    J_velocities.append(J_velocity)
...
print("++++++++  Initial : J_tot = ", J_tots[0], ", J_val = ", J_vals[0], ", J_pressure = ", J_pres[0], ", J_thermal = ", J_thes[0])
print("              ... : J_penalization = ", J_penalizations[0], ", J_velocity = ", J_velocities[0])
print("              ... : T_cold_in = ",avg_T_cold_in[0], ", T_hot_in = ",avg_T_hot_in[0],", T_cold_out = ",avg_T_cold_out[0],", T_hot_out = ",avg_T_hot_out[0], "\n")
for ii in range(len(steps_vector)-1):
    print("++++++++ Step ",steps_vector[ii+1]," : J_tot = ", J_tots[ii+1], ", J_val = ", J_vals[ii+1], ", J_pressure = ", J_pres[ii+1], ", J_thermal = ", J_thes[ii+1])
    print("              ... : J_penalization = ", J_penalizations[ii+1], ", J_velocity = ", J_velocities[ii+1])
    print("              ... : T_cold_in = ",avg_T_cold_in[ii+1], ", T_hot_in = ",avg_T_hot_in[ii+1],", T_cold_out = ",avg_T_cold_out[ii+1],", T_hot_out = ",avg_T_hot_out[ii+1], "\n")
