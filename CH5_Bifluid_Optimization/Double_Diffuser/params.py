...
length_y_wall_hole_in = 3.0e-3    # distance btw the wall and the inlet
length_y_wall_hole_out = length_y_wall_hole_in    # distance btw the wall and the outlet
length_y_hole_in = 15.0e-3    # size of the inlets
length_y_hole_out = length_y_hole_in    # size of the outlets
...
u_avg_in = Re * mu_f / (length_y_hole_in * rho_f)   # average inlet velocity [m/s]
...
r_filtering_eta2 = 0.01  # radius for the helmholtz filtering for the fluid contiguity penalization
r_filtering_eta1 = r_filtering_eta2    # radius for the helmholtz filtering at every outer cycle
start_with_filtering_eta1 = False

# scaling the costs
w_cost_weight = 0.15      # weight to balance the costs:  w = 1.0 -> J = J_pressure
                         #                               W = 0.0 -> J = J_thermal
...
length_y_wall_hole_in_adim = length_y_wall_hole_in / L_bar
length_y_wall_hole_out_adim = length_y_wall_hole_out / L_bar
length_y_hole_in_adim = length_y_hole_in / L_bar
length_y_hole_out_adim = length_y_hole_out / L_bar
...
iterations = [300, 150]
alpha_penalizations = [1.0e3, 1.0e4]  # inverse permeability of the solid
velocity_penalizations = [1.0e1, 1.0e1]
cost_intermediate_velocity = 1.0e2
## Convexity parameters
b_alpha =  [1.0, 1.0]    # conv. param. for the inverse permeability
